...
## Phase 1-A. 기반 설정
- Backend: Firebase Admin SDK 초기화, Firestore 데이터 모델 설계(User/Station/Pod), 성북구 정류장·역 데이터 시딩 (FR-5), ID 토큰 검증 미들웨어
- Frontend: Firebase Client SDK 설정, Google 로그인 버튼/플로우 (FR-1)
- 예정 API: `GET /api/stations`
- 검증: 로그인 성공, 정류장 목록 조회 가능
